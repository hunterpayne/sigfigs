...
class SignificantDigitsSpec 
    extends FlatSpec with Matchers with util.PrettyPrinter {

  behavior of "Significant Digits"
  ...
  it should "be automatically computed" in {
    ...
    digits.toString should be (toBold("23") + "00")
    digits1.toString should be (toBold("2300"))
    digits2.toString should be (toBold("230") + "0")
    digits3.toString should be (toBold("2300"))

    sig.toString should be (toBold("23") + "00")
    sig1.toString should be (toBold("24"))
    sig2.toString should be (toBold("24.303"))
    sig3.toString should be (toBold("2301.1"))
    sig4.toString should be (".000" + toBold("1223"))
    sig5.toString should be (".000" + toBold("122300"))
  }
  ...
  it should "handle basic byte math operations" in {
    ...
    sumDigits.toInt should be (124)
    sumDigits.toString should be (toBold("1") + "00")

    val subDigits = sig - sig1
    subDigits.digits should be (1)
    subDigits.toInt should be (76)
    subDigits.toString should be (toBold("8") + "0")

    val negDigits = -sig
    negDigits.digits should be (1)
    negDigits.toInt should be (-100)
    negDigits.toString should be ("-" + toBold("1") + "00")

    val multDigits = sig2 * sig3
    multDigits.digits should be (1)
    multDigits.toInt should be (72)
    multDigits.toString should be (toBold("7") + "0")

    val divDigits = sig2 / sig3
    divDigits.digits should be (1)
    divDigits.toInt should be (2)
    divDigits.toString should be (toBold("2"))
  }
  ...
  it should "handle basic float math operations" in {
    ...
    assert(Math.abs(sumDigits.toDouble - 124.33) < tolerance)
    sumDigits.toString should be (toBold("124.3"))

    val subDigits = sig - sig1
    subDigits.digits should be (3)
    assert(Math.abs(subDigits.toDouble - 75.87) < tolerance)
    subDigits.toString should be (toBold("75.9"))

    val negDigits = -sig
    negDigits.digits should be (4)
    assert(Math.abs(negDigits.toDouble + 100.1) < tolerance)
    negDigits.toString should be ("-" + toBold("100.1"))

    val multDigits = sig2 * sig3
    multDigits.digits should be (1)
    multDigits.toDouble should be (72.0)
    multDigits.toString should be (toBold("7") + "0")
  ...
  it should "handle basic double math operations" in {
    ...
    sumDigits.toInt should be (2324)
    sumDigits.toString should be (toBold("23") + "00")

    val subDigits = sig - sig1
    subDigits.digits should be (2)
    subDigits.toInt should be (2276)
    subDigits.toString should be (toBold("23") + "00")

    val negDigits = -sig
    negDigits.digits should be (2)
    negDigits.toInt should be (-2300)
    negDigits.toString should be ("-" + toBold("23") + "00")

    val multDigits = sig * sig1
    multDigits.digits should be (2)
    multDigits.toInt should be (55200)
    multDigits.toString should be (toBold("55") + "000")

    val divDigits = sig / sig1
    divDigits.digits should be (2)
    divDigits.toDouble should be (2300.0 / 24.0)
    divDigits.toString should be (toBold("96"))
  }
  ...
  it should "handle basic BigInt math operations" in {
    ...
    divDigits.toString should be (toBold("95"))
  }
  ...
  it should "handle double exponential and log math operations" in {
    ...
    assert(Math.abs(sinDigits.toDouble - 0.8919286509533796) < tolerance)
    sinDigits.toString should be (toBold(".892"))

    val cosDigits = cos(sig1)
    cosDigits.digits should be (3)
    assert(Math.abs(cosDigits.toDouble + 0.45217616214091194) < tolerance)
    cosDigits.toString should be (toBold("-.452"))

    val tanDigits = tan(sig1)
    tanDigits.digits should be (3)
    assert(Math.abs(tanDigits.toDouble + 1.9725247052617234) < tolerance)
    tanDigits.toString should be (toBold("-1.97"))

    val asinDigits = asin(sig2)
    asinDigits.digits should be (4)
    assert(Math.abs(asinDigits.toDouble - 0.5771) < tolerance)
    asinDigits.toString should be (toBold(".5771"))

    val acosDigits = acos(sig2)
    acosDigits.digits should be (4)
    assert(Math.abs(acosDigits.toDouble - 0.9937) < tolerance)
    acosDigits.toString should be (toBold(".9937"))

    val atanDigits = atan(sig2)
    atanDigits.digits should be (4)
    assert(Math.abs(atanDigits.toDouble - 0.49945882) < tolerance)
    atanDigits.toString should be (toBold(".4995"))

    val toRadiansDigits = toRadians(sig1)
    toRadiansDigits.digits should be (3)
    assert(Math.abs(toRadiansDigits.toDouble - 0.03560471674068433) < tolerance)
    toRadiansDigits.toString should be (".0" + toBold("356"))

    val toDegreesDigits = toDegrees(sig1)
    toDegreesDigits.digits should be (3)
    assert(Math.abs(toDegreesDigits.toDouble - 116.88339020668793) < tolerance)
    toDegreesDigits.toString should be (toBold("117"))

    val atan2Digits = atan2(sig1, sig2)
    atan2Digits.digits should be (3)
    assert(Math.abs(atan2Digits.toDouble - 1.3094618350270495) < tolerance)
    atan2Digits.toString should be (toBold("1.31"))

    val hypotDigits = hypot(sig, sig1)
    hypotDigits.digits should be (3)
    assert(Math.abs(hypotDigits.toDouble - 10034.300207368722) < tolerance)
    hypotDigits.toString should be (toBold("100") + "00")

    val ceilDigits = ceil(sig)
    ceilDigits.digits should be (5)
    assert(Math.abs(ceilDigits.toDouble - 10035.0) < tolerance)
    ceilDigits.toString should be (toBold("10035"))

    val floorDigits = floor(sig)
    floorDigits.digits should be (5)
    assert(Math.abs(floorDigits.toDouble - 10034.0) < tolerance)
    floorDigits.toString should be (toBold("10034"))

    val rintDigits = rint(sig)
    rintDigits.digits should be (5)
    assert(Math.abs(rintDigits.toDouble - 10034.0) < tolerance)
    rintDigits.toString should be (toBold("10034"))
  ...
  it should "handle float exponential and log math operations" in {
    ...
    ulpDigits.digits should be (1)
    assert(Math.abs(ulpDigits.toDouble) < tolerance)
    ulpDigits.toString should be (".000000" + toBold("2"))
  }
  ...
  it should "handle long exponential and log math operations" in {
    ...
    absDigits.toString should be (toBold("24"))

    val maxDigits = max(sig, sig1)
    maxDigits.digits should be (5)
    maxDigits.toLong should be (10034)
    maxDigits.toString should be (toBold("10034"))

    val minDigits = min(sig, sig1)
    minDigits.digits should be (2)
    minDigits.toLong should be (-24)
    minDigits.toString should be (toBold("-24"))
  ...
  it should "handle byte exponential and log math operations" in {
    ...
    maxDigits.toString should be (toBold("103"))
