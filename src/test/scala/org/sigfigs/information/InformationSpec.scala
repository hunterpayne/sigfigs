...
class InformationSpec extends FlatSpec with Matchers {
  ...
  it should "create values using UOM factories" in {
    Bytes(1).toBytes should be(SDD(1))
    Kilobytes(1).toKilobytes should be(SDD(1))
    Megabytes(1).toMegabytes should be(SDD(1))
    Gigabytes(1).toGigabytes should be(SDD(1))
    Terabytes(1).toTerabytes should be(SDD(1))
    Petabytes(1).toPetabytes should be(SDD(1))
    Exabytes(1).toExabytes should be(SDD(1))
    Zettabytes(1).toZettabytes should be(SDD(1))
    Yottabytes(1).toYottabytes should be(SDD(1))

    Kibibytes(1).toKibibytes should be(SDD(1))
    Mebibytes(1).toMebibytes should be(SDD(1))
    Gibibytes(1).toGibibytes should be(SDD(1))
    Tebibytes(1).toTebibytes should be(SDD(1))
    Pebibytes(1).toPebibytes should be(SDD(1))
    Exbibytes(1).toExbibytes should be(SDD(1))
    Zebibytes(1).toZebibytes should be(SDD(1))
    Yobibytes(1).toYobibytes should be(SDD(1))

    Bits(1).toBits should be(SDD(1))
    Kilobits(1).toKilobits should be(SDD(1))
    Megabits(1).toMegabits should be(SDD(1))
    Gigabits(1).toGigabits should be(SDD(1))
    Terabits(1).toTerabits should be(SDD(1))
    Petabits(1).toPetabits should be(SDD(1))
    Exabits(1).toExabits should be(SDD(1))
    Zettabits(1).toZettabits should be(SDD(1))
    Yottabits(1).toYottabits should be(SDD(1))

    Kibibits(1).toKibibits should be(SDD(1))
    Mebibits(1).toMebibits should be(SDD(1))
    Gibibits(1).toGibibits should be(SDD(1))
    Tebibits(1).toTebibits should be(SDD(1))
    Pebibits(1).toPebibits should be(SDD(1))
    Exbibits(1).toExbibits should be(SDD(1))
    Zebibits(1).toZebibits should be(SDD(1))
    Yobibits(1).toYobibits should be(SDD(1))
  }
  ...
  it should "properly convert to all supported Units of Measure" in {
    ...
    val big = scala.math.pow(1000, 6).toLong
    val x = Bytes(SDL(big))
    val x1 = Yottabytes(1l)

    x.toBytes should be(SDL(big))
    x.toKilobytes should be(SDL(scala.math.pow(1000, 5).toLong))
    x.toMegabytes should be(SDL(scala.math.pow(1000, 4).toLong))
    x.toGigabytes should be(SDL(scala.math.pow(1000, 3).toLong))
    x.toTerabytes should be(SDL(scala.math.pow(1000, 2).toLong))
    x.toPetabytes should be(SDL(scala.math.pow(1000, 1).toLong))
    x.toExabytes should be(SDL(1))
    //x1.toZettabytes should be(SDL(1000))
    //x1.toYottabytes should be(SDL(1))

    x.toKibibytes should be(SDL(scala.math.floor(big / BinarySystem.Kilo).toLong, 1))
    x.toMebibytes should be(SDL(scala.math.floor(big / BinarySystem.Mega).toLong, 1))
    x.toGibibytes should be(SDL(931322574l, 1))
    x.toTebibytes should be(SDL(909494l, 1))
    x.toPebibytes should be(SDL(scala.math.floor(big / BinarySystem.Peta).toLong, 1))
    //x.toExbibytes should be(SDL(1))
    //x1.toZebibytes should be(SDL(1024))
    //x1.toYobibytes should be(SDL(1))

    //x.toBits should be(SDL(big * 8l))
    x.toKilobits should be(SDL(8 * scala.math.pow(1000, 5).toLong, 1))
    x.toMegabits should be(SDL(8 * scala.math.pow(1000, 4).toLong, 1))
    x.toGigabits should be(SDL(8 * scala.math.pow(1000, 3).toLong, 1))
    x.toTerabits should be(SDL(8 * scala.math.pow(1000, 2).toLong, 1))
    x.toPetabits should be(SDL(8000l))
    x.toExabits should be(SDL(8l))
    //x1.toZettabits should be(SDL(8 * scala.math.pow(1000, 1).toLong, 1))
    //x1.toYottabits should be(SDL(8l))

    x.toKibibits should be(SDL(scala.math.floor(big / BinarySystem.Kilo / Bits.conversionFactor).toLong, 1))
    x.toMebibits should be(SDL(scala.math.floor(big / BinarySystem.Mega / Bits.conversionFactor).toLong, 1))
    x.toGibibits should be(SDL(7450580596l, 1))
    x.toTebibits should be(SDL(7275957l, 1))
    x.toPebibits should be(SDL(scala.math.floor(big / BinarySystem.Peta / Bits.conversionFactor).toLong, 1))
    x.toExbibits should be(SDL(6l))
    //x1.toZebibits should be(SDL(8192l))
    //x1.toYobibits should be(SDL(8l))
  }
