...
import terra._
import terra.time._
import terra.space.{ Meters, SquareMeters }
import terra.motion.{ MetersPerSecond, MetersPerSecondCubed, MetersPerSecondSquared }
import terra.radio.SquareMeterSeconds
...
class TimeSpec extends FlatSpec with Matchers {
  ...
  it should "create values using UOM factories" in {
    Nanoseconds(1).toNanoseconds should be(SDD(1))
    Microseconds(1).toMicroseconds should be(SDD(1))
    Milliseconds(1).toMilliseconds should be(SDD(1))
    Seconds(1).toSeconds should be(SDD(1))
    Minutes(1).toMinutes should be(SDD(1))
    Hours(1).toHours should be(SDD(1))
    Days(1).toDays should be(SDD(1))
  }
  ...
  it should "return equality for units in different units" in {
    ...
    Hours(24) should be(Days(SDD(1, 2)))
  }

  it should "return Time when added to Time" in {
    Seconds(60) + Minutes(1) should be(Seconds(SDD(120, 2)))
    Minutes(15) + Hours(1.75) should be(Hours(2))
  }

  it should "return Time when subtracted to Time" in {
    Minutes(5) - Seconds(30) should be(Minutes(SDD(4.5, 1)))
    Hours(5) - Minutes(30) should be(Hours(SDD(4.5, 1)))
    Days(5) - Hours(12) should be(Days(SDD(4.5, 1)))
  }

  it should "properly convert to all supported Units of Measure" in {
    val x = Seconds(1)
    x.toNanoseconds should be(SDD(1e9))
    x.toMicroseconds should be(SDD(1e6))
    x.toMilliseconds should be(SDD(1000))
    x.toSeconds should be(SDD(1))
    x.toMinutes should be(SDD(1d / 60, 1))
    x.toHours should be(SDD(1d / 3600, 1))
    x.toDays should be(SDD(1d / (3600 * 24), 1))
  }
