...
import terra._
import terra.time.{ Seconds, Hertz }
...
class DimensionlessSpec extends FlatSpec with Matchers {
  ...
  it should "create values using UOM factories" in {
    Percent(1).toPercent should be(SDD(1))
    Each(1).toEach should be(SDD(1))
    Dozen(1).toDozen should be(SDD(1))
    Score(1).toScore should be(SDD(1))
    Gross(1).toGross should be(SDD(1))
  }
  ...
  it should "properly convert to all supported Units of Measure" in {
    val x = Gross(1)
    x.toPercent should be(SDD(14400, 1))
    x.toEach should be(SDD(144, 1))
    x.toDozen should be(SDD(12, 1))
    x.toScore should be(SDD(144d / 20, 1))
    x.toGross should be(SDD(1))
  }
  ...
  it should "return another Dimensionless when added to a Double" in {
    Each(10) + 10.22 should be(Each(SDD(20.22, 1)))
  }
  ...
  it should "provide implicit conversion from Double" in {
    ...
    val coeff: DimensionlessConversionsI[SDD] = coefficient
